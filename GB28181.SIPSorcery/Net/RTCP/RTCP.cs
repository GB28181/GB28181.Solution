    ...
    public class RTCPReport
    {
        ...
        public byte[] GetBytes()
        {
            ...
            if (BitConverter.IsLittleEndian)
            {
                ...
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(TotalPackets)), 0, payload, 24, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(OutOfOrder)), 0, payload, 28, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(JitterAverage)), 0, payload, 32, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(JitterMaximum)), 0, payload, 36, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(JitterDiscards)), 0, payload, 40, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(PacketsLost)), 0, payload, 44, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(Duplicates)), 0, payload, 48, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(BytesReceived)), 0, payload, 52, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(TransmissionRate)), 0, payload, 60, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(Duration)), 0, payload, 64, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(AverageTransitTime)), 0, payload, 72, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(ReportNumber)), 0, payload, 76, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(LastReceivedReportNumber)), 0, payload, 80, 4);
            }
            else
            {
    ...
    public class RTCPReportSampler
    {
        ...
        private RTCPReport Sample(UInt16 sampleStartSequenceNumber, UInt16 sampleEndSequenceNumber, TimeSpan sampleDuration)
        {
            try
            {
                ...
                {
                    ReportNumber = m_reportNumber,
                    SampleStartTime = DateTime.MinValue,
                    //sample.SampleEndTime = DateTime.MinValue;
                    StartSequenceNumber = sampleStartSequenceNumber,
                    Duration = Convert.ToUInt64(sampleDuration.TotalMilliseconds)
                };
                    ...
                    if (m_rcvdSeqNums.ContainsKey(testSeqNum))
                    {
                        ...
                        else
                        {
                            ...
                            if (measurement.Jitter > sample.JitterMaximum)
                            {
                                ...
                                sample.JitterMaximum = measurement.Jitter;
                            }
                        }
