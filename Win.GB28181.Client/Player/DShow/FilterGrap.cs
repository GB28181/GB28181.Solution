    ...
    public class FilterGraphBase
    {
        ...
        public virtual bool ConnencAll(AMMediaType mediaType)
        {
            ...
            if (mediaType == null)
            {
                ...
                _DebugEx.Trace("DShow", string.Format("Grapth {0} to {1} {2}", firstFilterEx.Name, lastConnFilterEx.Name, bFlag ? "成功" : "失败"));
            }
            else
            {
            ...
            foreach (var filterEx in this.FilterExList.Skip(2))
            {
                ...
                _DebugEx.Trace("DShow", string.Format("Grapth{0}to{1} {2}", lastConnFilterEx.Name, filterEx.Name, bFlag ? "成功" : "失败"));
                lastConnFilterEx = filterEx;
            }
            if (!bFlag)
                foreach (var filterEx in FilterExList)
                    GraphBuilder.RemoveFilter(filterEx.BaseFilter);
            return bFlag;
        }
