...
namespace SLW.ClientBase.Mixer
{
    public sealed class MediaMixerCanvas
    {
        ...
        public void Dispaly(int count, int width, int height)
        {
            ...
            if (count < 0 || count > MAXVIDEO_NUM)
                return;
            Image image = Image.FromFile(@"D:\Mixer\SlWClient\SLW.WPFClient\Images\bg_module_call.png");
            mapRects.Clear();
            CreateRect(count);
        ...
        public void Play(Win.MediaNetEngine.MediaFrame frame)
        {
            //if(frame.StreamID)
